...
| Browser → Server | `POST /snapshots/clear-workspace` | Delete all snapshot files inside a workspace, leaving the directory in place. Body: `{ workspace: "..." }`. Validates workspace (safe-name check, must exist). Deletes all `*_screen.json` files in the directory; directory itself is preserved. Returns `{ ok: true, deleted: N }`. (v0.12) |
...
| U7e | History panel — delete functionality (v0.12): four delete operations available from the history panel: **(1) Single delete** — each snapshot row has a per-row delete button; confirms with the server and removes the row. **(2) Multi-select + delete** — user enters selection mode (via recycle bin header button); checkboxes appear on all rows across all workspace sections; a "Delete selected" action deletes all checked items. **(3) "Clear workspace"** — deletes all snapshot files inside a specific workspace; the workspace accordion row remains visible but empty (folder kept on disk). Available as a per-workspace action in selection mode. **(4) "Workspace delete"** — deletes the workspace folder and all its snapshot files; the accordion row disappears from the panel entirely. Requires confirmation dialog. All deletes are permanent (no undo, see K1–K2). | v0.12 |
| U7f | History panel — header layout (v0.12): a recycle bin (trash) icon button is added to the history panel header. This button activates selection/delete mode (triggering the behaviour described in U7e). All action buttons (recycle bin, lock/unlock from U7b) are aligned to the right of the header. A vertical separator (`\|`) divides the action button group from the close button, which remains in the far-right corner. Layout (left→right): `[title/spacer] [recycle-bin] [lock/unlock] | [close]`. | v0.12 |
