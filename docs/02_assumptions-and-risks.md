...
> ✅ RESOLVED (found 2026-07-18, during Sprint 71 manual browser verification): `@scope` only matches elements within the scope root's own subtree. Bootstrap's entire color system (alert/badge/table variants, etc.) is defined via CSS custom properties set on `:root` — an ancestor of the scope root, never a descendant — so those rules silently never matched inside `@scope (#anchor) { ... }`, and every color/background depending on them fell back to its initial value (transparent/black) while non-variable properties (padding, etc.) kept working, with no error surfaced anywhere. Not caught by Sprint 70's unit tests, which only checked for CSS-block presence/scoping-string shape, not actual computed styles. Fixed by rewriting `:root` → `:scope` in the Bootstrap CSS text before wrapping (`:scope` inside an `@scope` block refers to the scope root element itself; custom properties set there inherit normally to every descendant) — applied in both `server/export-html.ts` (patched retroactively) and `client/src/renderers/Html.svelte`. Confirmed live in a real browser (Playwright/Chromium) before and after the fix.
...
**N6 — NF14 closed drift at the broadcast-construction layer only; command-handler business logic and per-transport input validation were out of scope (found 2026-07-09, re-audit — see `01` Design Debt Log, `docs/raw/design-problems.md`)**
> ✅ RESOLVED (2026-07-09, `Milestone_v0.27.md` Sprints 51–57): all 7 findings closed — NF18 (F1, slideshow validation parity), NF19 (F2, step/seek extracted into `render-core.ts`), NF20 (F3, workspace mandatory for `list_snapshots`/`GET /snapshots`), NF21 (F4, `export-html` unifies on `ids`), NF22 (F5, canonical `getSnapshotsRoot()`), NF23 (F6, reused `isFrameArray()`), NF24 (F7, shared zod schemas for `node_actions`/`node_to_frame`).
> ⚠️ ASSUMPTION (v0.23–v0.26) INVALIDATED: NF14–NF17 were assumed to fully realize `render-core.ts`'s own documented intent ("used identically by REST and MCP ... so the two transports can never drift", NF12) for every command — not just the five (`render`, `init/append/commit_step_frames`) already routed through it. A follow-up REST↔MCP duplication audit (F1–F7, `docs/raw/design-problems.md`) found this incomplete:
- **F2** — `step`/`seek` business logic was never extracted into `render-core.ts`; it is still copied verbatim between `app.ts` and `mcp.ts`.
- **F1** — MCP's `slideshow` tool bypasses `validate.ts`'s `validateFrame()` entirely, hand-rolling a subset of the same checks — a direct violation of that function's own documented "no second implementation" invariant.
- **F3, F4** — two findings are outright behavioral divergence, not just duplicated code: `list_snapshots` (MCP) vs `GET /snapshots` (REST) resolve `workspace` differently, and `export-html`'s item-addressing shape differs between transports.
- **F5, F6, F7** — three more mechanical duplications (snapshots-root path resolution reimplemented 9×, a Frame-shape predicate reimplemented 3×, and REST/MCP input-shape validation expressed via two different mechanisms) that don't yet diverge in behavior but carry the same drift risk NF14 was built to eliminate.

**Lesson (same shape as C2e, one layer down):** a shared module only closes drift for the call sites migrated to use it at the time it was built. Nothing structural stops a call site the migration missed — or business logic that was never extracted in the first place — from reimplementing the same rule independently. Scheduled for remediation in `Milestone_v0.27.md` — see `03` §8, `04` §9 additions, `05` registry.

**N7 — `app.ts` accumulated non-routing responsibilities the F1–F7 audit couldn't see, because it was scoped to REST↔MCP drift, not intra-REST bloat (found 2026-07-10, manual review — see `01` FR23)**
> ✅ RESOLVED (2026-07-10, `Milestone_v0.28.md` Sprints 58–61) for items 1–4: NF25 (item 1, `applyLoadedSnapshotResult` in `render-core.ts`), NF26 (item 2, `snapshot-writer.ts` gains `deleteSnapshotFiles`/`deleteWorkspace`), NF27 (item 3, remaining workspace-validation paths consolidated onto `validateWorkspaceInput()`), NF28 (item 4, `isValidSnapshotFilename()` in `validate.ts`). Item 5 remains open — see below, unchanged, explicitly deferred to a future intake (`03` §9's scope decision).
>
> ⚠️ ASSUMPTION (v0.27) INVALIDATED: closing N6/Milestone v0.27 was implicitly assumed to leave `app.ts` as a "thin REST adapter" per D5 (`04` §9.3). Not true — five responsibilities were identified that didn't belong directly in the route-handler file: (1) `POST /snapshots/load`'s inline commit logic (same shape as `render-core.ts`'s commit functions, but REST-only so invisible to F1–F7's REST↔MCP comparison), (2) snapshot file deletion + workspace-path containment checking with no owning module, (3) four independently-implemented workspace-validation code paths *within* `app.ts` itself, (4) a filename-safety regex copy-pasted twice, (5) HTTP status derived by substring-matching an error message instead of the neutral `{ok,error,category?}` shape `04` §9.3 already documents as a target-architecture contract change that was never actually implemented anywhere.
>
> **Lesson (same shape as N6, one layer down again):** N6 found NF14's projector unification only covered REST↔MCP *transport* drift. This finds the same "coverage gap" pattern one layer further in: a file can be internally inconsistent (four validation paths, a copy-pasted regex) without ever showing up as REST/MCP drift, because the duplication is *within* one transport, not *between* two. Auditing "does X exist in both files" misses "does X exist in 4 different forms in one file."
>
> **Follow-up lesson, found during implementation (2026-07-10):** consolidating item 3 onto `validateWorkspaceInput()` (which lived in `render-core.ts`) meant `snapshot-writer.ts` (item 2's new module) needed to import it — but `render-core.ts` already imports `snapshot-writer.ts` for `generateSnapshotId()`, so this created a two-module import cycle. It didn't fail `tsc --noEmit` (type-checking doesn't evaluate module load order), but deadlocked Vitest's `vi.mock`/`importOriginal()` during test collection — the test run hung with near-zero CPU rather than erroring, which took longer to diagnose than the fix itself. Resolved by relocating `validateWorkspaceInput()` (and the new `isValidSnapshotFilename()`, item 4) to `validate.ts`, a true leaf module (`04` §9.7 has the full writeup). Worth remembering for future cross-module consolidations in this codebase: a shared helper's new home must stay a leaf relative to *all* its callers, not just the one that prompted moving it — `tsc` will not catch a violation, only a mocked test run (or careful dependency-graph review) will.
>
> **Item 5 — scope is unbounded until scoped, still open:** implementing the neutral `{ok,error,category}` shape "properly" (per `04` §9.3's original intent) means touching every error-returning path in both `app.ts` and `mcp.ts`, not just the two `"not found or expired"` string-matches that prompted the finding — this could grow from a small cleanup into a full error-contract migration. Needs an explicit scope decision before scheduling into a future milestone.
>
> **Consideration — item 3's consolidation must preserve call-site-specific reaction, not just the check itself:** the four workspace-validation paths differ in more than error text — `/export-html`'s loop *silently skips* an invalid workspace (filtering a list), while `/render`/`/slideshow`/`/snapshots` *reject the whole request*. A single shared validator must let each caller choose reject-vs-skip; it can't hardcode one behavior the way `validateWorkspaceInput` currently does (always returns a reject-shaped result).

**N8 — Client-side responsibility bloat is the same pattern as N7, one tier over: found in the browser components, not the server files (found 2026-07-12, manual review — see `01` FR24, `docs/06_frontend-desing-review.md`)**
> ✅ RESOLVED (2026-07-12, `Milestone_v0.29.md` Sprints 62–65): NF29 (pan/zoom camera → `renderers/mermaid/panZoom.ts`), NF29+NF30 (click routing + node-to-frame wiring → `renderers/mermaid/nodeInteractions.ts`; popup UI → `renderers/mermaid/NodeActionPopup.svelte`), NF31 (`formatTimestamp()` → `lib/formatTimestamp.ts`; snapshot-row display → `lib/SnapshotRow.svelte`), NF32 (`DeleteExportModal.svelte`'s delete/export fetch calls → `lib/snapshotActions.ts`). All four landed as output-equivalence refactors — full unit suite (484 tests) and the 38-test Playwright e2e suite passed unchanged after each sprint; `tsc --noEmit` and `npm run lint` clean throughout.
>
> The F1–F7 and N7 audits were both server-scoped (REST↔MCP drift, then intra-`app.ts` bloat). Neither looked at `client/src/`. A direct review of the four largest client files found the same two failure shapes recurring one tier over: `Mermaid.svelte` (580 lines) mixes four responsibilities — Mermaid rendering, a generic pan/zoom camera, click-to-server-action routing plus a popup UI, and node→frame navigation wiring — behind a name that promises only the first; and `formatTimestamp()` plus the snapshot-row display (markup+CSS) are independently duplicated between `HistoryPanel.svelte` and `DeleteExportModal.svelte`, the same "no owning module" gap N7 found for server-side deletes.
>
> ⚠️ ASSUMPTION: splitting `Mermaid.svelte`'s pan/zoom camera and click-routing/popup UI into their own modules (`panZoom.ts`, `nodeInteractions.ts`, `NodeActionPopup.svelte`) is being justified purely on readability/naming grounds — today there is exactly one consumer (Mermaid) for all three. This is deliberately *not* being framed as "extracting a reusable pan/zoom widget for future renderers" (no other renderer needs one yet, per `renderers/registry.ts`); if that framing crept in during implementation it would be speculative generality ahead of a real second caller. The extraction is justified because it lets `Mermaid.svelte`'s remaining code actually match its name, independent of reuse.
>
> **Risk — regression surface during the split:** the pan/zoom and click-routing code being extracted is stateful, DOM-listener-heavy, and has accumulated several real bug fixes over time (B12 focus-trap, B16 dev-proxy, B17 SVG-sizing, B19/FR21 per-frame fit). A mechanical split that changes attach/detach timing or listener-cleanup order could silently reintroduce one of those. Mitigated by the existing Playwright e2e coverage for pan/zoom, click routing, and node-to-frame (same suite that caught B16/B17/B19 originally) — any refactor here must keep that suite green, not just `tsc`/unit tests, since the historical bugs in this exact code were browser-layout/timing issues invisible to both.
