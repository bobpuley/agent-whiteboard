# Milestone v0.27 — REST/MCP Parity Remediation (Sprints 51–53)
...
### Sprint 51 — Close documented-invariant violations (F1, F2)
- [ ] **NF18 — `slideshow` MCP tool routes through `validateFrame()`.** Remove the hand-rolled mermaid/vega-lite checks in `mcp.ts`'s `slideshow` tool (`mcp.ts:256-299`) and call `validate.ts`'s `validateFrame()` for every slide, matching REST's `/slideshow` handler (`app.ts:219`).
...
- [ ] **NF19 — Extract `step`/`seek` into `render-core.ts`.** Move the viewport-lookup + `resolvedId` + broadcast logic currently duplicated between `app.ts:114-175` and `mcp.ts:95-194` into shared functions in `render-core.ts`, and have both `POST /step`/`POST /seek` and the `step()`/`seek()` MCP tools call them.
...
### Sprint 52 — Behavior-contract parity (F3, F4)
- [ ] **NF20 — Workspace mandatory, no fallback, for `list_snapshots`/`GET /snapshots`.** Remove `GET /snapshots`'s `getLastWorkspace()` fallback in `app.ts:370-383`; workspace becomes a required parameter, matching MCP's `list_snapshots`.
...
- [ ] **NF21 — `export-html` unifies on `ids`.** Remove REST's `{workspace, filename}` item shape and the hand-parsed filename-lookup branch in `app.ts:653-696`; both transports accept only `{workspace, ids}`.
...
### Sprint 53 — Mechanical dedup (F5, F6, F7)
- [ ] **NF22 — Canonical snapshots-root resolver.** New `server/paths.ts` exporting one function wrapping `WHITEBOARD_SNAPSHOTS_DIR ?? join(homedir(), ".agent-whiteboard")`; replace all 9 independent copies (`app.ts` ×5 incl. `resolveSnapshotRoot()`, `mcp.ts` ×3, `viewport-cache.ts`, `snapshot.ts`).
  - *Acceptance:* every call site imports the shared resolver; behavior (resolved path) unchanged for all existing tests.
- [ ] **NF23 — Reuse `snapshot-reader.ts`'s `isFrameArray()`.** Remove the two inline re-definitions in `app.ts` (`450-451`, `666-667`); import and use the existing exported predicate instead.
  - *Acceptance:* no behavior change; existing tests covering `/snapshots/load` and `/export-html` pass unchanged.
- [ ] **NF24 — Share `node_actions`/`node_to_frame` validation via MCP's zod schemas.** Export MCP's existing zod schemas for these two shapes from a shared location; REST's `app.ts` replaces `isNodeActionsValid()`/`isNodeToFrameValid()` with `.safeParse()` calls against the same schemas.
...
> **Implementation note:** NF20/NF21 are the only two tasks in this milestone with browser-visible surface area — sequence them after NF18/NF19 land and are verified, and test the HistoryPanel manually (not just e2e) before closing Sprint 52, consistent with this project's "verify UI changes in a real browser" convention.
