...
| `append_frame` | `append_frame(id, payload, label?)` | Append one frame to an in-progress step-frames sequence identified by `id`. `payload` is validated against the sequence's `frame_type` (same hard gate as `render()`). `label` is an optional display caption for this frame. Returns `{ "ok": true, "frame_count": N }` where N is the current total number of frames. Error if the `id` is unknown/expired or the payload fails validation. Does NOT update the browser — call `commit_step_frames()` to render. | v0.8 |
| `commit_step_frames` | `commit_step_frames(id)` | Finalise an in-progress step-frames sequence and render it. Assembles the full step-frames JSON from all appended frames, writes a snapshot, broadcasts to the browser via WebSocket (identical effect to `render(type="step-frames", ...)`). Cancels any running slideshow (same as `render()`). Returns `{ "ok": true }`. Error if the `id` is unknown/expired or the sequence has zero frames. Clears the in-memory entry for this `id` after commit. After commit, `export()` returns the assembled full step-frames JSON — same contract as after a direct `render(type="step-frames", ...)` call. `clear()` during an active builder session does NOT cancel the builder entry; the TTL handles cleanup if the agent abandons it. | v0.8 |
...
| F13 | **All-workspaces snapshot list:** `GET /snapshots/all` — scans every subdirectory of `WHITEBOARD_SNAPSHOTS_DIR`, reads each workspace's snapshots, and returns them grouped. Response: `{ ok: true, workspaces: [{ name, isCurrent, snapshots: [{ filename, timestamp, type, title? }] }] }`. Each workspace's snapshot list is sorted newest-first. `isCurrent: true` historically set by `WHITEBOARD_WORKSPACE` env var — **open question (v0.7):** with env var removed (FR4), how `isCurrent` is determined must be decided (options: drop field; derive from last `render()` call in-memory; client localStorage). See G2c. Workspaces with no readable snapshots are omitted. Returns `{ ok: true, workspaces: [] }` if root absent. | v0.5; isCurrent TBD v0.7 |
