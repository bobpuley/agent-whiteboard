...
| Dev server | Separate Vite dev server (`localhost:5173`) + Node server (`localhost:3000`); started together via `concurrently`; Vite proxies `/render`, `/stream`, `/mcp` to Node | HMR on Svelte side; Node server implementation unchanged; production static build deferred to Phase 2 |
...
- [ ] Configure Vite proxy: `/render`, `/stream`, `/mcp` → `localhost:3000`
...
- [ ] **Verify** that Claude Code auto-loads `.mcp.json` from the project root on open (see `02` §7 assumption U4); if not, document the manual step required
