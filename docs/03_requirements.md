...
| F17 | **Export to self-contained HTML (`POST /export-html`, v0.13):** accepts a list of `{ workspace, filename }` pairs (from the HistoryPanel "Export selected" button in selection mode). Server reads each snapshot, renders its content server-side, and assembles a single self-contained HTML file. Rendering strategies: Mermaid → inline SVG via `mermaid.render()` in a `happy-dom` Window; KaTeX → HTML string via `katex.renderToString(source, { displayMode: true, throwOnError: false })`; Vega-Lite → inline SVG via `vl.compile(spec).spec` → `vega.parse()` → `new vega.View().toSVG()`; SVG payloads → sanitized with DOMPurify (`USE_PROFILES: { svg: true }`) in the same `happy-dom` Window; HTML payloads → sanitized with DOMPurify (`USE_PROFILES: { html: true }`); step-frames → expanded per frame, each frame rendered by its `frame_type` (recursive, same table). Per-item render failure: content area shows an inline error message; export continues. One `happy-dom` Window per request, torn down after all items are rendered. Document structure: `<nav>` (table of contents with workspace/item hierarchy) + `<main>` (workspace `<section>`s → item `<section>`s → optional frame sub-sections for step-frames). Ordering: items within a workspace sorted chronologically oldest-first (by `timestamp` field); workspaces ordered by their earliest item's timestamp. All CSS is inline; KaTeX CSS is included only when the export contains ≥1 KaTeX items. No external JS or CSS references — file opens correctly offline. New npm dependency: `happy-dom` (server-side only). | v0.13 |
...
| U7e | History panel — delete functionality (v0.12; Clear workspace removed v0.13): three delete operations available from the history panel: **(1) Single delete** — each snapshot row has a per-row delete button; confirms with the server and removes the row. **(2) Multi-select + delete** — user enters selection mode (via recycle bin header button); checkboxes appear on all rows across all workspace sections; a "Delete selected" action deletes all checked items. **(3) "Workspace delete"** — deletes the workspace folder and all its snapshot files; the accordion row disappears from the panel entirely. Requires confirmation dialog. All deletes are permanent (no undo, see K1–K2). "Clear workspace" (files deleted, directory preserved, empty row remains) is removed in v0.13 — it has the same high-level effect as "Workspace delete". | v0.12; simplified v0.13 |
| U7f | History panel — header layout (v0.12; updated v0.13): a recycle bin (trash) icon button is added to the history panel header; this button enters delete mode (triggering the behaviour described in U7e). All action buttons are right-aligned; a vertical separator divides them from the close button. v0.12 layout: `[title/spacer] [recycle-bin] [lock/unlock] \| [close]`. v0.13 layout (adds export icon): `[title/spacer] [recycle-bin] [export] [lock/unlock] \| [close]`. | v0.12; layout updated v0.13 |
| U7g | History panel — export mode (v0.13): a dedicated export/download icon button in the panel header (adjacent to the recycle bin "Edit" button) enters export mode. Export mode and delete mode (U7e) share the same checkboxes-on-rows UI but have distinct actions. In export mode: **(1) "Export selected"** — appears in the select-bar, enabled when ≥1 items are checked; POSTs `{ items: [{ workspace, filename }] }` to `POST /export-html` and triggers a browser download via a temporary `<a download>` element. **(2) "Export workspace"** — appears on each workspace accordion header; immediately exports all snapshots in that workspace via `POST /export-html` with all its items (no checkbox selection required — analogous to "Workspace delete" in delete mode). Mode-switching: clicking the recycle bin while in export mode (or the export icon while in delete mode) switches modes immediately without a cancel step. | v0.13 |
