...
**Promoted to v1.3/v1.4/v1.5 (2026-09-06)** — see `02`/`03`/`04` §5–7 and `Milestone_v1.3.md`/`Milestone_v1.4.md`/`Milestone_v1.5.md` (NF45–NF54): `nodeToFrame` unchecked-cast-from-disk, export-HTML global-state isolation, dev-mode client port/proxy `PORT` mismatch (→ v1.3); request body size limit, sync→async fs I/O (→ v1.4); explicit `crypto` import, typed `channel.ts` notification interface, `PORT`/`CHANNEL_PORT` validation, `app.ts` route-file split, vite/vitest major upgrade (→ v1.5).
