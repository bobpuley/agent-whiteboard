...
| House style for `type: "html"` content (live view + export) | Bootstrap 5, CSS-only build, `bootstrap` npm dependency (planned v0.31, FR25 in `01`, F20 in `03`) | CSS read from `bootstrap/dist/css/bootstrap.min.css`, same "npm dependency, read at render/export time" pattern as the `mermaid.js` embedding above — chosen over a manually vendored copy for one versioning mechanism across both embedded assets. Component-class framework (cards, alerts, badges, tables) chosen over classless (Pico/Water — no composite-widget primitives) and utility-class (Tailwind — no "just mention it" property, needs an exact class vocabulary plus normally a build/purge step); Bootstrap chosen over Bulma specifically for LLM training-data familiarity. CSS-only — no JS bundle, since `html`-type payloads are static rendered content, not interactive widgets. Scoped via the same `@scope`/`scopeEmbeddedStyles()` mechanism as B20 (see the export pseudocode below and `03` F20) so bare-element rules never leak into the export's own chrome or the live app's own UI. Live view: lazy-loaded inside `Html.svelte`, not eagerly bundled — see the `renderers/` tree note below. Risk: extends `@scope`'s (already-shipped, previously unlogged) browser-support gap to a much larger stylesheet — accepted, see L7 in `02`. |
...
user enters export mode → clicks "Export workspace" on a workspace accordion header
  ...
  → assemble HTML document:
      ...
      → <style>: layout CSS always included; KaTeX CSS included only when ≥1 katex items present;
        Bootstrap CSS (planned v0.31, `03` F20) included, wrapped in the same @scope mechanism as
        scopeEmbeddedStyles() above, only when ≥1 html items present
...
│   │       ├── Html.svelte  # planned v0.31 (FR25 in `01`, F20 in `03`): lazy-loads Bootstrap 5 CSS-only, scoped via @scope, only when a type:"html" payload mounts — same lazy-load precedent as Mermaid/KaTeX/Vega-Embed above.
│   │       ├── Katex.svelte
│   │       └── VegaLite.svelte
│   └── public/
...
| 3 | Four independently-implemented "is this workspace acceptable" checks within `app.ts` | Consolidated onto the existing `validateWorkspaceInput()` — no new unit, just removing the 3 ad-hoc reimplementations. Relocated from `render-core.ts` to `validate.ts` during implementation (not originally planned): `snapshot-writer.ts`'s new delete functions (item 2) also need it, and `render-core.ts` already imports `snapshot-writer.ts` for `generateSnapshotId()` — keeping `validateWorkspaceInput()` in `render-core.ts` would have made `snapshot-writer.ts` import back from `render-core.ts`, a two-module cycle that deadlocked Vitest's `vi.mock`/`importOriginal()` during test collection. `validate.ts` is a true leaf (only `presentation.ts` beneath it), so both `render-core.ts` and `snapshot-writer.ts` can depend on it one-directionally; `render-core.ts` re-exports it for its existing callers (`app.ts`, `mcp.ts`), so no call site's import path changed. | NF27 ✅ |
| 4 | Snapshot-filename safety regex copy-pasted twice | One exported helper, `isValidSnapshotFilename()`. Colocated with `validate.ts` (not `snapshot-writer.ts` as originally sketched) alongside the sibling `isValidWorkspaceName()`/`validateWorkspaceInput()` — same leaf-module reasoning as item 3, and keeps `app.ts`'s `/snapshots/load` handler from needing an otherwise-unnecessary `snapshot-writer.ts` import just for a pure string predicate | NF28 ✅ |
