...
The Svelte/Vite client has its own `tsconfig.json` generated by `create svelte` (also `strict: true`), but — found during code review, 2026-07-04, see B10 in `01` — it was never wired into the build's type-check gate: root `tsconfig.json` excludes `client/`, and `npm run build` only runs `tsc` against `server/`. **Planned fix (v0.18):** add `svelte-check` as a dev dependency and a `typecheck` script (`svelte-check --tsconfig client/tsconfig.json`), and run it as part of `build`/CI, giving the client the same enforced-strict guarantee the server already has.
