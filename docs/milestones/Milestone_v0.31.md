...
- [ ] Scope the injected stylesheet to the component's own container (`.html-renderer`) via `@scope`, using the Sprint 69 helper, so it never affects the rest of the app's chrome (history panel, controls, other renderer instances).
- [ ] Manual verification in the browser: an `html`-type `render()` call with Bootstrap classes (`card`, `alert`, `badge`) renders styled; an `svg`-type payload triggers no Bootstrap network/parse cost; the app's own UI (buttons, panels) is visually unchanged before and after an `html` payload is shown.
- [ ] Unit test in `tests/unit/client/Html.test.ts`: Bootstrap stylesheet is injected/loaded for `type: "html"`, not for `type: "svg"`.
