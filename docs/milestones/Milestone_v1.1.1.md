...
- [x] **Add `'unsafe-eval'` to `script-src` in `server/app.ts`'s `CSP_HEADER`.** Change `"script-src 'self' 'unsafe-inline'"` to `"script-src 'self' 'unsafe-inline' 'unsafe-eval'"`. Leave every other directive (`default-src`, `style-src`, `img-src`, `connect-src`, `object-src`, `base-uri`, `frame-ancestors`) untouched.
  ...
  - **Shipped 2026-08-04:** `CSP_HEADER` in `server/app.ts` now grants `'unsafe-eval'` in `script-src`; every other directive unchanged. Added a rationale line to the existing CSP comment block pointing at B23/F31. New unit test `tests/unit/server/app.test.ts` ("Content-Security-Policy header") asserts `script-src` contains `'self'`, `'unsafe-inline'`, and `'unsafe-eval'`. `server/export-html.ts`'s two CSP strings confirmed untouched. Full suite: 529 unit tests passing (up from 528), `tsc --noEmit`/`svelte-check`/`eslint` clean. Live-browser verification of showcase slides "6 / 6 — Vega-Lite" and "7b — Vega-Lite (6 s)" pending human-driven test pass.
