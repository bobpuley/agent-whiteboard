...
>
> **Risk — item 5's scope is unbounded until scoped:** implementing the neutral `{ok,error,category}` shape "properly" (per `04` §9.3's original intent) means touching every error-returning path in both `app.ts` and `mcp.ts`, not just the two `"not found or expired"` string-matches that prompted the finding — this could grow from a small cleanup into a full error-contract migration. Needs an explicit scope decision before scheduling (see `03` for the resolution).
>
> **Consideration — render-core.ts gains a REST-only function:** the proposed 4th commit function (item 1) has no MCP caller today (no `load_snapshot` tool exists). `render-core.ts`'s header comment currently states it's "used identically by REST and MCP" (NF12) — technically still true (nothing stops MCP from calling it later), but worth an explicit note that not every function in this module needs an MCP caller to belong there; its job is composing *canvas commands*, not proving transport parity.
>
