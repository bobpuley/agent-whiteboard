...
- **Explicit `crypto` import (NF50):** `server/snapshot-writer.ts` now imports `randomUUID` explicitly from `node:crypto` instead of relying on the bare global, matching `server/export-html.ts`'s existing pattern.
- **Typed channel notification (NF51):** `server/channel.ts` defines a minimal `NotifiableServer` interface for the one MCP `notification()` method it needs instead of casting to `any`; a rejected notification is now logged instead of silently swallowed.
- **Port validation (NF52):** a new `server/port.ts` `parsePort()` helper validates `PORT` (`server/index.ts`) and `CHANNEL_PORT` (`server/channel.ts`, `server/app.ts`), failing fast with a clear error instead of a low-level bind/fetch error. `0` (OS-assigned ephemeral port) is deliberately accepted, matching `Number.isInteger(port) && port >= 0 && port <= 65535` — the existing test suite already relies on `PORT=0` for ephemeral test binding.
- **`app.ts` route split (NF53):** route registrations move out of `createApp()` into `server/routes/{render,slideshow,snapshots,export}.ts`, each exporting a `(app: Hono) => void` registration function — no behavior change, verified against the full REST/MCP suite.
- **`vite`/`vitest` upgrade (NF54):** `vite` 4→5, `@sveltejs/vite-plugin-svelte` 2→3, `vitest` 0.34→3.2 — the latest versions still compatible with this project's Svelte 4 (the vite/vitest literal-latest majors require a Svelte 5 migration via `vite-plugin-svelte` 4+/5+, out of scope here). Found and fixed a real latent XSS risk uncovered along the way: `dompurify` 3.4.8+ silently fails to sanitize against a `happy-dom` `Window` (upstream incompatibility, capricorn86/happy-dom#1810), which would have broken sanitization in both the client test suite and the production `/export-html` endpoint once `dompurify`'s floating version moved past 3.4.7. Replaced `happy-dom` with `jsdom` everywhere (client test environment + `server/export-html.ts`'s DOMPurify window); `renderHtmlPayload()` also gained explicit `ADD_TAGS: ["style"]` + `FORCE_BODY: true`, since the B20 embedded-`<style>`-scoping feature turned out to depend on the same happy-dom leniency rather than an explicit DOMPurify allowlist.
- Full suite: 586 unit tests passing (up from 573), `tsc --noEmit`/`svelte-check`/`eslint`/`npm run build` clean.
...
- **Sprint 75 (NF33/NF34):** `createApp()` gains an opt-in `staticRoot` option that serves the built `dist/client` directly via `@hono/node-server/serve-static`, mounted last so it never shadows an API route — dev mode (`npm run dev`, Vite dev server + proxy) is unaffected. `server/index.ts`'s startup logic is extracted into an exported `startServer({ staticRoot, onReady })`, guarded behind an is-main-module check so it can be called as a library without double-starting. New `bin/cli.js` (plain ESM, no build step) is the production entrypoint `npx agent-whiteboard` runs — resolves `dist/client` relative to its own file location (cwd-independent) and opens the browser via the existing `open` dependency, importing nothing outside `dependencies`.
- **Sprint 76 (F27/F28):** `startServer()` now refuses to bind a non-loopback `HOST` unless `ALLOW_NON_LOOPBACK=1` is explicitly set, turning the accepted single-user/local-only trust model into an enforced guarantee instead of a documentation-only default. `NodeActionPopup.svelte`'s keyboard trap (no `Escape`-reachable dismiss path — HIGH finding in `docs/06_frontend_review.md`) is fixed by reusing the existing `trapFocus` action, and its action items now activate on `Space` as well as `Enter`.
- **Sprint 77 (NF35/NF36):** `package.json` gains `license`, `description`, `repository`, `homepage`, `bugs`, `author`, `keywords`, a `bin` field (`agent-whiteboard` → `bin/cli.js`), and a `files` allowlist (`bin`, `dist`, `README.md`, `LICENSE`, `CHANGELOG.md`) — `private: true` is removed and `version` becomes `1.0.0`. `npm pack --dry-run` now ships 163 files (down from 213 unfiltered) and `npm publish --dry-run` succeeds. `README.md` gains an `npx agent-whiteboard` quickstart and an explicit trust-model statement.
- **Sprint 78 (NF37):** new `.github/workflows/ci.yml` runs `npm ci`, typecheck, lint, test, and build on every push/PR to `master` — no deploy/publish automation, publishing stays a manual, deliberate step.
- **Deferred (Design Debt Log in `01`):** all MEDIUM-severity findings from the Node.js/TS and Svelte/TS review passes (body-size limits, WS auto-reconnect, `export-html`'s global-state serialization, etc.) — deliberately out of scope for this milestone, logged as candidates for a future one.
- Full suite: 525 unit tests passing (up from 513 pre-milestone), `tsc --noEmit`/`svelte-check`/`eslint` clean throughout.
...
- **Sprint 66 (B20):** `.item-section`/`.frame-section` gained `overflow-x: auto` so an oversized table/code block scrolls within its own section instead of overflowing the page — a real fix, but manual testing showed it wasn't the actual cause of the "narrow main" report. The real cause: an `html`-type payload's own embedded `<style>` tag (e.g. a markdown→HTML theme's `body { max-width: 900px }`) is document-scoped, not scoped to its `.item-section`, and was silently overriding the whole export's layout. An initial `FORBID_TAGS: ["style"]` fix closed the leak but also discarded the payload's own legitimate table/code formatting living in the same `<style>` block; replaced with `scopeEmbeddedStyles()`, which wraps any surviving `<style>` tag's content in `@scope (#<item-or-frame-id>) { ... }` — containing selector *matching* to that item/frame's own subtree without discarding anything
- **Sprint 67 (B21):** `table` gained `border-collapse: collapse`, removing the default `border-collapse: separate` + `border-spacing` gap that visibly offset a table's own outer border from its cells' borders
- **Sprint 68 (B22):** `assembleHtml()`'s TOC now expands a step-frames item into one nav submenu link per frame (`<ul class="toc-frames">`), titled with the frame's own label (falling back to "Frame N"); the item's own (parent) link points at frame 0's anchor, matching `<main>`'s existing per-frame sub-sections
- All three root-caused and verified via direct Playwright reproduction (headless Chromium) before and after each fix, in addition to new unit test coverage in `tests/unit/server/export-html.test.ts`
- Full suite: 489 unit tests passing (up from 486 pre-milestone), 38/38 e2e tests, `tsc --noEmit` and `npm run lint` clean throughout. `03` §5d and `04`'s HTML Export data-flow notes updated from gap to resolved

## 0.27.2 — 2026-07-12

**Milestone v0.29 — Client Component Responsibility Cleanup (Sprints 62–65) complete.** A direct review of the four largest `client/src/` files (`docs/06_frontend-desing-review.md`, `01` FR24) found the same "responsibilities hidden inside one file" pattern `Milestone_v0.28.md` cleaned up server-side (`02` §N7), one tier over in the browser components (`02` §N8). All four items landed as output-equivalence refactors — no user-visible behavior change.

- **Sprint 62 (NF29 part 1):** the pan/zoom camera (`scale`/`tx`/`ty`/`dragging` state, `fitToView`, `applyViewport`, wheel/mouse handlers, `resetTransform`, debounced `/viewport` reporting) extracted out of `renderers/Mermaid.svelte` into `renderers/mermaid/panZoom.ts` as a standalone Svelte-store-backed module
- **Sprint 63 (NF29 part 2 + NF30):** click-to-server-action routing (`attachClickListeners`/`detachClickListeners`, `onNodeClick`/`onEdgeClick`, `POST /node-click`) and node-to-frame navigation wiring (`attachNodeToFrameListeners`/`detachNodeToFrameListeners`, `POST /seek`) extracted into `renderers/mermaid/nodeInteractions.ts`; the node-action popup menu (markup + styles) extracted into `renderers/mermaid/NodeActionPopup.svelte`, with only its open/closed state remaining in `Mermaid.svelte`
- **Sprint 64 (NF31):** `formatTimestamp()`, byte-identical in `HistoryPanel.svelte` and `DeleteExportModal.svelte`, unified into `lib/formatTimestamp.ts`; the snapshot title/type-badge/timestamp display (markup + CSS, also duplicated between the same two files) unified into `lib/SnapshotRow.svelte`, used by both
- **Sprint 65 (NF32):** `DeleteExportModal.svelte`'s inline fetch logic for `/snapshots/delete-workspace`, `/snapshots/delete-files`, and `/export-html` (including response parsing and error-message extraction) moved into `deleteWorkspace`/`deleteFiles`/`exportItems` in a new `lib/snapshotActions.ts` — the component now contains no direct `fetch()` calls, only step/UI orchestration
- `renderers/Mermaid.svelte` shrank from 580 lines to just the Mermaid source→SVG rendering pipeline and Mermaid-DOM-parsing helpers (`extractNodeId`/`extractNodeLabel`/`extractEdgeId`); no change to `renderers/registry.ts` or any external API/WebSocket contract
- Full suite: 484 unit tests passing (up from 457 pre-milestone — new coverage in `panZoom.test.ts`, `nodeInteractions.test.ts`, `NodeActionPopup.test.ts`, `formatTimestamp.test.ts`, `SnapshotRow.test.ts`, `snapshotActions.test.ts`), 38/38 e2e tests, `tsc --noEmit` (client) and `npm run lint` clean throughout. `02` §N8, `03` §10, `04` §9.8 updated from open/scheduled to resolved
