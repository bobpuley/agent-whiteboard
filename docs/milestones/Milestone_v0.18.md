...
- [x] **T4 (B9, MEDIUM) — `client/src/App.svelte`:** wrap `handleDone()`'s `fetch` in `try`/`catch`; on failure, leave `doneSent` false and surface an error state (or allow retry) instead of an unhandled rejection.
- [x] **T5 (B10, MEDIUM) — build tooling:** add `svelte-check` as a dev dependency, add a `typecheck` script (`svelte-check --tsconfig client/tsconfig.json`), wire it into `npm run build`. Fix `HistoryPanel.svelte:24`'s invalid `res.json<T>()` call as part of closing the gap (`const data = (await res.json()) as {...}`).
  - Added `client/svelte.config.js` (exports `vitePreprocess()`) — required for `svelte-check` to resolve TS-in-`<script>` preprocessing outside a SvelteKit project; without it, every `.svelte` file failed with "No Svelte configuration found in vite config".
  - Turning on real client type-checking surfaced two more genuine (pre-existing, previously invisible) type gaps beyond the known `res.json<T>()` call, both fixed here: (1) `ws.ts`'s `RenderCommand`'s primary "replace" variant typed `type` as bare `string` instead of the actual literal union of renderer types, which meant TypeScript couldn't discriminate it from the `"step-frames-placeholder"` variant — fixed by introducing an exported `RendererType` literal union; (2) `App.svelte`'s step-bar visibility check (`canvas.type !== "empty" && canvas.stepFrames`) didn't also exclude the `"step-frames-placeholder"` `CanvasState` variant, which has no `stepFrames` field — added the missing exclusion. Neither was an observed runtime bug (both degraded gracefully via JS's tolerance for undefined property access), but both are exactly the class of error this task exists to catch going forward.
  - Verified `npm run build` exits non-zero on a deliberately introduced client-only type error, and exits 0 / builds successfully once fixed. Full unit (223) + e2e (31) suites pass with no regressions.
