...
**FR24 — Client component responsibility audit: Mermaid.svelte / HistoryPanel.svelte / DeleteExportModal.svelte / App.svelte (2026-07-12)**
Raw request: audit four client files (`renderers/Mermaid.svelte` 580 lines, `HistoryPanel.svelte` 364 lines, `DeleteExportModal.svelte` 687 lines, `App.svelte` 365 lines) so that each file implements only its own responsibilities, common functions are shared instead of duplicated, and file/module names reflect what they're for. Full findings and proposed refactor: `docs/06_frontend-desing-review.md`. Findings summary:
- **F1 (🔴)** — `Mermaid.svelte` bundles four responsibilities: Mermaid source→SVG rendering, a generic pan/zoom camera with server-side viewport persistence, click-to-server-action routing plus a popup menu UI, and autonomous node→frame navigation wiring. Only rendering and Mermaid-DOM-parsing helpers are actually Mermaid-specific.
- **F2 (🟡)** — `formatTimestamp()` is byte-identical, duplicated in `HistoryPanel.svelte` and `DeleteExportModal.svelte`.
- **F3 (🟡)** — the snapshot-row display (title + type badge + timestamp), markup and CSS, is duplicated near-verbatim between the same two files.
- **F4 (🟢)** — the "POST JSON, swallow network error" fetch pattern is hand-rolled independently at every call site across `Mermaid.svelte`, `HistoryPanel.svelte`, and `DeleteExportModal.svelte` instead of going through a shared helper.
- Not findings: `App.svelte` (clean orchestrator, no violation) and `DeleteExportModal.svelte`'s dual delete/export `mode` (a parameterized shared shell, not copy-paste duplication — splitting it would duplicate the step-1/step-2 shell instead of removing duplication).
Proposed refactor (see review doc for full rationale): `lib/formatTimestamp.ts` (resolves F2), `lib/SnapshotRow.svelte` (resolves F3), split `Mermaid.svelte` into rendering-only + `renderers/mermaid/panZoom.ts` + `renderers/mermaid/nodeInteractions.ts` + `renderers/mermaid/NodeActionPopup.svelte` (resolves F1), `lib/snapshotActions.ts` (resolves F4 on the modal side). Explicitly out of scope: `App.svelte` changes, splitting `DeleteExportModal` into two components, and a generic fetch helper for Mermaid's three call sites (deferred, low value at three sites).

**B20 — Export HTML `<main>` content column is too narrow on wide viewports (found 2026-07-15, user report, tested file `study-coach_algorithms-20260713-201859.html`)**
- Observed: opening the exported HTML on a desktop with ample horizontal screen space shows `<main>` rendering as a narrow column with large unused whitespace to its right, instead of using the available viewport width (`./narrow_section.png`). As a consequence, some elements inside item/frame sections — tables, code blocks — extend visually past their `.item-section`/`.frame-section` border rather than wrapping or scrolling within it (`./table_exceeds_border.png`); the user notes widening `<main>` should resolve both.
- Expected: `<main>` should make effective use of available viewport width on wide screens rather than sitting at a narrow fixed-feeling column, and no contained element (table, code block, etc.) should ever visually exceed its enclosing section's border — it should wrap or scroll internally instead.

**B21 — Some export tables have unaligned right-side row borders (found 2026-07-15, user report, same export file)**
- Observed: in the exported HTML, some tables render with a right-side row border that doesn't line up with the table's outer border / other rows (`./unaligned_right_border.png`).
- Expected: table borders should render as a clean, uniformly aligned grid — every row's right edge flush with the table's outer border, no drift between rows.

**B22 — Left-menu TOC has no per-frame submenu for multi-frame (step-frames) snapshots (found 2026-07-15, user report, same export file)**
- Observed: for a step-frames snapshot, the left `<nav>` table of contents shows a single item-level entry linking to the item's section, even though `<main>` already breaks that item into per-frame sub-sections (F17 in `03`). There is no way to jump to a specific frame from the nav.
- Expected: a step-frames item should get one submenu entry per frame in the left-menu nav, each with its own title (the frame's label, falling back to "Frame N"), nested under that item's TOC entry — the item's own (parent) link should point at frame 0's anchor, i.e. behave identically to that first submenu entry.

**FR25 — Bootstrap house style for `type: "html"` content, shared between live view and export (2026-07-15)**
Raw request: provide a small, meaningful set of styles for `html`-type content so an agent can build visually consistent, deterministic layouts without hand-authoring CSS every time; consider adopting a well-known CSS framework rather than a bespoke one, since "we just need to mention it, and the agent already knows it." The same style set should also apply to exported snapshots. Prompted by inspecting a real export (`study-coach_algorithms-20260713-201859.html`, the same file that surfaced B20–B22) which showed the agent reinventing its own mini design system per payload — hand-rolled `.note`/`.good`/`.warn` callout boxes, `.tag` badges, `.leg`/`.li`/`.lb` legends, and a `.cell`/`.n`/`.s`/`.m`/`.today` calendar-heatmap grid, on top of hand-styled `h2`/`h3`/`table`/`th`/`td`/`pre`.

Stress-tested via `/grill-me`; resolved decisions:
- **Scope:** `type: "html"` payloads only — not KaTeX/Mermaid/Vega-Lite's own rendering-library styling, not the export's own `<nav>`/`<main>` chrome.
- **Framework shape:** component-class, not classless/semantic (e.g. Pico/Water) and not utility-class (Tailwind) — the real usage shows domain-specific composite widgets (callouts, badges, legends, grids), which no classless framework's bare-tag styling covers, and utility classes have no "just mention it" property (needs an exact, memorized class vocabulary plus normally a build/purge step).
- **Framework choice: Bootstrap, CSS-only build (no JS bundle).** Chosen over Bulma specifically for LLM training-data familiarity ("agent already knows it" — the actual goal, not just technical merit). No JS is shipped since this is static rendered content, not interactive widgets; components that need Bootstrap's JS (dropdowns, modals, tooltips, popovers, collapses, carousels, offcanvas) render static markup but won't function — the agent should avoid them.
- **Scoping mechanism:** reuse the existing `@scope`/`scopeEmbeddedStyles()` mechanism shipped v0.30 for B20, so Bootstrap's bare-element resets (`h1`–`h6`, `table`, `code`, etc.) apply only inside `type: "html"` payload containers — never to the export's own section headings/nav or the live app's own UI chrome.
- **Source of truth:** `bootstrap` npm dependency, CSS read at render/export time — same pattern as the existing `mermaid.js` embedding (L1/L6 in `02`), not a manually vendored copy.
- **Export inclusion:** conditional — included in the export's `<style>` block only when the export contains ≥1 `html`-type item (expanded through step-frames), mirroring the existing KaTeX-CSS/Mermaid-bundle conditional-inclusion pattern (F17 in `03`).
- **Live-view loading:** lazy-loaded inside `Html.svelte` only when `type === "html"` actually mounts — matches the documented precedent in `renderers/registry.ts` for other heavy third-party libraries (Mermaid/KaTeX/Vega-Embed), including its explicit note about a prior bundle-size regression from loosening this discipline.
- **Agent-facing documentation:** the `render` MCP tool's `"html"` description line (`server/mcp.ts`) gets a fuller rewrite — names Bootstrap 5 explicitly, gives concrete class examples (`alert alert-info`, `card`, `badge`, `table table-striped`), and states the CSS-only/no-JS-components caveat.
- **Deliberately out of scope:** no bespoke component library (e.g. a first-class "calendar heatmap" or "legend" component) — recurring domain-specific visuals stay agent-authored custom markup/`<style>` (already safely scoped by the mechanism above), promoted to a named component only if the same pattern repeats enough across real exports to justify it later.
- **Risk surfaced and accepted:** extending `@scope` to a full ~160–200KB framework stylesheet (not just a small per-payload `<style>` block) raises the blast radius of `@scope`'s browser-support gap (Chrome/Edge 118+, Safari 17.4+, Firefox 128+) — previously unlogged even for its original B20 use. In an unsupported browser the whole `@scope` block is dropped by the CSS parser (graceful degrade to unstyled semantic HTML, not corruption/leak). Accepted as-is — see L7 in `02`.

Propagated into `02` (new L7), `03` (new F20), `04` (new dependency row + export/`Html.svelte` notes), and `Milestone_v0.31.md`.
