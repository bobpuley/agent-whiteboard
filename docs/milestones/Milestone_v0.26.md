...
- [x] **Run the migration against real data.** Back up `~/.agent-whiteboard/` (54 snapshots across 8 workspaces as of 2026-07-07, see `02` N5), run the script against the backup copy first in a dry-run mode, verify output, only then run for real. *Acceptance: a real backup exists before the real run; the real run is verified against the backup afterward.*
  - Run 2026-07-08: real directory had grown to 161 snapshot files (35 already in the new schema, written by dev/e2e test runs during Sprints 42–43's own development). Backup taken at `~/.agent-whiteboard.backup-20260708-205116/`. Dry run against the live directory (safe — dry-run never writes) previewed 126 migrated / 35 already-migrated / 0 errors. A real run against a throwaway copy was verified byte-for-byte against the backup (id/timestamp/workspace/title/nodeToFrame/frames/rawPayload all preserved, 0 mismatches across all 161 files, 0 pre-migration files missing an `id`) before running for real against the live directory. Post-run: diffed against the backup again (0 mismatches), confirmed idempotency (re-run reports 0 migrated / 161 already-migrated / 0 errors), and smoke-tested end-to-end against the real data with a live server (`GET /snapshots`, `POST /snapshots/load`, `GET /export` all correct against a migrated `study-coach_core-java` snapshot).

### Sprint 45 — MCP payload contract update
> Depends on the data model (Sprint 39) and WS contract (Sprint 42).
- [x] **Update the MCP payload contract.** Remove `type:"step-frames"` as a top-level content type (a multi-frame Presentation replaces it); keep `render`/`slideshow`/`step`/`seek`/etc. as ergonomic sugar over "commit a Presentation + set a driver" per D5. Update `README.md`, MCP tool schemas/descriptions, and `tests/human_driven/showcase.js` to the new contract. No back-compat shim (`02` N4 — pre-release, zero external consumers). *Acceptance: showcase script runs clean end-to-end against the new contract; README's MCP tool surface table matches the shipped schema exactly; MCP-payload-shape tests are rewritten against the new contract.*
  - Shipped 2026-07-09: `render()` is single-frame only (`type: "step-frames"` removed from its enum, `options.node_to_frame` removed); `init_step_frames`/`append_frame`/`commit_step_frames` is now the sole multi-frame creation path, with `node_to_frame` moved to an optional `commit_step_frames(id, node_to_frame?)` parameter (its only entry point now — U4e). `slideshow()` slides are single-frame only (the step-frames-slide-expansion-into-timer-ticks behavior is removed). `validatePayload()`/`KNOWN_TYPES` deleted from `validate.ts` — every content path calls `validateFrame()` directly. `persist.ts`'s `PersistableContent`/`toFrames()` conversion replaced with a direct `frames: Frame[]` + `rawPayload?` shape. Server unit tests rewritten (423 passing, down from 444 — redundant one-shot-vs-incremental duplicate tests consolidated) plus e2e spec (`tests/e2e/canvas.spec.ts`) updated to build step-frames sequences via the incremental REST protocol. `README.md`, `03_requirements.md`, and `04_architecture.md` updated to describe the shipped contract, with historical sections annotated rather than rewritten. `showcase.js` verified end-to-end against a live `npm run dev:test` server (Sections 1, 7, 8, 12, 13, 14).
